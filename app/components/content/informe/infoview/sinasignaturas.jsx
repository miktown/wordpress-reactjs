...
class InformePass extends React.Component {
  ...
  listPasswords () {
    ...
    piezas.sort((a, b) => b.colegio_id - a.colegio_id).map(password => {
      if (password.isasignaturas === false && password.colegio_zona.id && password.colegio_zona.id > 0 && this.isValidZona(password.colegio_zona.id)) {
        this.csv.data.push(
        ...
        passOutput.push(<div style={{background: 'white', margin: '.5em auto', padding: '.5em 1em', display: 'block', width: '100%', boxShadow: '0 1px 2px #999', height: 'auto', position: 'relative', overflow: 'hidden'}} key={password.id}>
          <p style={{marginLeft: '2em', float: 'left', width: '450px'}}><a href={password.url}>{password.name}</a></p>
          <p style={{marginLeft: '2em', float: 'left'}}> id: {password.colegio_id} </p>
  ...
  render () {
    ...
    return <div>
      <p className='introduction'>
        {dataPiezas.length} {dataPiezas.length === 1 ? 'clase' : 'clases'} sin asignaturas <strong>(no le afecta el filtro de fechas)</strong>
