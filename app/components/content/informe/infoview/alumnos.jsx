...
class InformeAlumnos extends React.Component {
  ...
  getClasesIn (clases, alumnoId) {
    let output = []
    clases.map( clase => {
      output.push(<li  style={{boxShadow:'none',margin:0}} key={alumnoId + '_' + clase.clase}>- <a href={clase.url} target="_blank">{this.decodeHtml(clase.name)}</a></li>)
    })
    ...
    .map(alumno => {
      ...
      alumnosOutput.push(<li style={{padding: '0.5em 0.5em 0.1em'}} key={alumno.id}>
        ...
        <p style={{marginBottom: '1.5em'}} className='studentIcon leftGo'>
          <span style={{color: '#333', fontWeight: '800'}}><a target='_blank' href={alumno.url}>{alumno.nombre}</a> ({alumno.colegio.nombre})</span> <strong className={alumno.tipo}>{this.setAlumnoCurso(alumno.curso.nombre)}</strong>
        ...
        <ul>{this.getClasesIn(alumno.clases, alumno.id)}</ul>
      </li>)
    })
